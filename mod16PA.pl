...
test_female :- female(mary), female(edith), 
				\+female(travis), female(zeus).

% A parent is either one of a persons biological parents
...
step_father(X,Y) :- spouse(Z,Y), male(Y), parent(X,Z), \+parent(X,Y).
step_mother(X,Y) :- spouse(Z,Y), female(Y), parent(X,Z), \+parent(X,Y).
...
aunt(X, Y) :- female(Y), sibling(Y, Z), parent(X, Z).
aunt(X, Y) :- female(Y), spouse(Y, W), sibling(W, Z), parent(X, Z).
